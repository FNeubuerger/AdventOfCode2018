...
fn find_first_duplicate(numbers: &[i32]) ->i32{
    let mut res = 0;
    let mut res_vec = Vec::new();
    let mut condition = false ;
    let mut duplicate = 0;
    let mut counter = 1;
    res_vec.push(0); // this gives 0 as the start
    'outer: while condition == false {
        'inner: for i in 0..numbers.len(){
                    ...
                    if condition == true { //if the resulting number is already in the array append it for display reasons and break loop
                        res_vec.push(res);
                        duplicate = *res_vec.last().unwrap();
                        break 'outer
                    } else { // append the number and continue
                        res_vec.push(res);
                        continue;
                    }
                }
